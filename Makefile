...
uninstall:
	python3 -m pip uninstall -y rio-xpw
...
pylint:
	pylint $(shell git ls-files rio_xpw/*.py)
flake8:
	flake8 rio_xpw --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 rio_xpw --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics
pytest:
	pytest --cov=rio_xpw --cov-report=term-missing --cov-report=xml --cov-report=html --cov-config=.coveragerc --cov-fail-under=100
pytest-clean:
	rm -rf .pytest_cache
